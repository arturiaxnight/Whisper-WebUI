...
class App:
    def __init__(self, args):
        ...
        self.nllb_inf = NLLBInference(model_dir=self.args.nllb_model_dir, output_dir=os.path.join(self.args.output_dir, "translations"))
        self.deepl_api = DeepLAPI(output_dir=os.path.join(self.args.output_dir, "translations"))
    ...
    def create_whisper_parameters(self):
        ...
        with gr.Row():
            cb_timestamp = gr.Checkbox(value=whisper_params["add_timestamp"], label="Add a timestamp to the end of the filename", interactive=True)
        with gr.Accordion("Advanced Parameters", open=False):
            nb_beam_size = gr.Number(
                label="Beam Size", value=whisper_params["beam_size"], precision=0, interactive=True, info="Beam size to use for decoding."
            )
            nb_log_prob_threshold = gr.Number(
            ...
            nb_no_speech_threshold = gr.Number(
            ...
            nb_best_of = gr.Number(
                label="Best Of",
                value=whisper_params["best_of"],
                interactive=True,
                info="Number of candidates when sampling with non-zero temperature.",
            )
            nb_patience = gr.Number(label="Patience", value=whisper_params["patience"], interactive=True, info="Beam search patience factor.")
            cb_condition_on_previous_text = gr.Checkbox(
                label="Condition On Previous Text",
                value=whisper_params["condition_on_previous_text"],
                interactive=True,
                info="Condition on previous text during decoding.",
            )
            sld_prompt_reset_on_temperature = gr.Slider(
                ...
                step=0.01,
                interactive=True,
                info="Resets prompt if temperature is above this value." " Arg has effect only if 'Condition On Previous Text' is True.",
            )
            tb_initial_prompt = gr.Textbox(label="Initial Prompt", value=None, interactive=True, info="Initial prompt to use for decoding.")
            sd_temperature = gr.Slider(
                label="Temperature",
            ...
            nb_compression_ratio_threshold = gr.Number(
            ...
            nb_chunk_length = gr.Number(
                label="Chunk Length (s)",
                value=lambda: whisper_params["chunk_length"],
                precision=0,
            ...
            with gr.Group(visible=isinstance(self.whisper_inf, FasterWhisperInference)):
                nb_length_penalty = gr.Number(
                    label="Length Penalty", value=whisper_params["length_penalty"], info="Exponential length penalty constant."
                )
                nb_repetition_penalty = gr.Number(
                ...
                nb_no_repeat_ngram_size = gr.Number(
                    label="No Repeat N-gram Size",
                    value=whisper_params["no_repeat_ngram_size"],
                    precision=0,
                    info="Prevent repetitions of n-grams with this size (set 0 to disable).",
                )
                tb_prefix = gr.Textbox(
                    label="Prefix", value=lambda: whisper_params["prefix"], info="Optional text to provide as a prefix for the first window."
                )
                cb_suppress_blank = gr.Checkbox(
                    label="Suppress Blank", value=whisper_params["suppress_blank"], info="Suppress blank outputs at the beginning of the sampling."
                )
                tb_suppress_tokens = gr.Textbox(
                ...
                nb_max_initial_timestamp = gr.Number(
                ...
                cb_word_timestamps = gr.Checkbox(
                    label="Word Timestamps",
                    value=whisper_params["word_timestamps"],
                ...
                tb_prepend_punctuations = gr.Textbox(
                ...
                tb_append_punctuations = gr.Textbox(
                ...
                nb_max_new_tokens = gr.Number(
                    label="Max New Tokens",
                    value=lambda: whisper_params["max_new_tokens"],
                    precision=0,
        ...
        with gr.Accordion("BGM Separation", open=False):
            cb_bgm_separation = gr.Checkbox(label="Enable BGM Separation Filter", value=uvr_params["is_separate_bgm"], interactive=True)
            dd_uvr_device = gr.Dropdown(
                label="Device", value=self.whisper_inf.music_separator.device, choices=self.whisper_inf.music_separator.available_devices
            )
            dd_uvr_model_size = gr.Dropdown(label="Model", value=uvr_params["model_size"], choices=self.whisper_inf.music_separator.available_models)
            nb_uvr_segment_size = gr.Number(label="Segment Size", value=uvr_params["segment_size"], precision=0)
            cb_uvr_save_file = gr.Checkbox(label="Save separated files to output", value=uvr_params["save_file"])
            cb_uvr_enable_offload = gr.Checkbox(label="Offload UVR model after separation", value=uvr_params["enable_offload"])

        with gr.Accordion("VAD", open=False):
            cb_vad_filter = gr.Checkbox(label="Enable Silero VAD Filter", value=vad_params["vad_filter"], interactive=True)
            sd_threshold = gr.Slider(
                minimum=0.0,
                maximum=1.0,
                step=0.01,
            ...
            nb_min_silence_duration_ms = gr.Number(
            ...
            nb_speech_pad_ms = gr.Number(
                label="Speech Padding (ms)",
                precision=0,
                value=vad_params["speech_pad_ms"],
        ...
        with gr.Accordion("Diarization", open=False):
            ...
            tb_hf_token = gr.Text(
                ...
                value=diarization_params["hf_token"],
            ...
            dd_diarization_device = gr.Dropdown(
                label="Device", choices=self.whisper_inf.diarizer.get_available_device(), value=self.whisper_inf.diarizer.get_device()
            )
        ...
        return (
            WhisperParameters(
                ...
                is_bgm_separate=cb_bgm_separation,
                uvr_device=dd_uvr_device,
                uvr_model_size=dd_uvr_model_size,
                uvr_segment_size=nb_uvr_segment_size,
                uvr_save_file=cb_uvr_save_file,
                uvr_enable_offload=cb_uvr_enable_offload,
            ),
            dd_file_format,
            cb_timestamp,
        )
        ...
        with self.app:
            ...
            with gr.Tabs():
                with gr.TabItem("File"):  # tab1
                    ...
                    btn_run.click(
                        fn=self.whisper_inf.transcribe_file, inputs=params + whisper_params.as_list(), outputs=[tb_indicator, files_subtitles]
                    )
                ...
                with gr.TabItem("Youtube"):  # tab2
                    ...
                    btn_run.click(
                        fn=self.whisper_inf.transcribe_youtube, inputs=params + whisper_params.as_list(), outputs=[tb_indicator, files_subtitles]
                    )
                ...
                with gr.TabItem("Mic"):  # tab3
                    ...
                    btn_run.click(
                        fn=self.whisper_inf.transcribe_mic, inputs=params + whisper_params.as_list(), outputs=[tb_indicator, files_subtitles]
                    )
                    btn_openfolder.click(fn=lambda: self.open_folder("outputs"), inputs=None, outputs=None)
                ...
                with gr.TabItem("T2T Translation"):  # tab 4
                    ...
                    with gr.TabItem("DeepL API"):  # sub tab1
                        ...
                        with gr.Row():
                            dd_source_lang = gr.Dropdown(
                                label="Source Language", value=deepl_params["source_lang"], choices=list(self.deepl_api.available_source_langs.keys())
                            )
                            dd_target_lang = gr.Dropdown(
                                label="Target Language", value=deepl_params["target_lang"], choices=list(self.deepl_api.available_target_langs.keys())
                            )
                        with gr.Row():
                            cb_is_pro = gr.Checkbox(label="Pro User?", value=deepl_params["is_pro"])
                        with gr.Row():
                            cb_timestamp = gr.Checkbox(
                                value=translation_params["add_timestamp"], label="Add a timestamp to the end of the filename", interactive=True
                            )
                        with gr.Row():
                    ...
                    btn_run.click(
                        ...
                        inputs=[tb_api_key, file_subs, dd_source_lang, dd_target_lang, cb_is_pro, cb_timestamp],
                        outputs=[tb_indicator, files_subtitles],
                    )

                    btn_openfolder.click(fn=lambda: self.open_folder(os.path.join(self.args.output_dir, "translations")), inputs=None, outputs=None)
                    ...
                    with gr.TabItem("NLLB"):  # sub tab2
                        with gr.Row():
                            ...
                            dd_source_lang = gr.Dropdown(
                                label="Source Language", value=nllb_params["source_lang"], choices=self.nllb_inf.available_source_langs
                            )
                            dd_target_lang = gr.Dropdown(
                                label="Target Language", value=nllb_params["target_lang"], choices=self.nllb_inf.available_target_langs
                            )
                        with gr.Row():
                    ...
                    btn_run.click(
                        ...
                        inputs=[file_subs, dd_model_size, dd_source_lang, dd_target_lang, nb_max_length, cb_timestamp],
                        outputs=[tb_indicator, files_subtitles],
                    )
                ...
                with gr.TabItem("BGM Separation"):
                    ...
                    dd_uvr_model_size = gr.Dropdown(
                        label="Model", value=uvr_params["model_size"], choices=self.whisper_inf.music_separator.available_models
                    )
                    nb_uvr_segment_size = gr.Number(label="Segment Size", value=uvr_params["segment_size"], precision=0)
                    cb_uvr_save_file = gr.Checkbox(label="Save separated files to output", value=True, visible=False)
                    ...
                    btn_open_instrumental_folder.click(
                        inputs=None, outputs=None, fn=lambda: self.open_folder(os.path.join(self.args.output_dir, "UVR", "instrumental"))
                    )
                    btn_open_vocals_folder.click(
                        inputs=None, outputs=None, fn=lambda: self.open_folder(os.path.join(self.args.output_dir, "UVR", "vocals"))
                    )

        # Launch the app with optional gradio settings
        args = self.args

        self.app.queue(api_open=args.api_open).launch(
            share=args.share,
...
parser.add_argument("--faster_whisper_model_dir", type=str, default=FASTER_WHISPER_MODELS_DIR, help="Directory path of the faster-whisper model")
parser.add_argument(
    "--insanely_fast_whisper_model_dir", type=str, default=INSANELY_FAST_WHISPER_MODELS_DIR, help="Directory path of the insanely-fast-whisper model"
)
parser.add_argument("--diarization_model_dir", type=str, default=DIARIZATION_MODELS_DIR, help="Directory path of the diarization model")
parser.add_argument("--nllb_model_dir", type=str, default=NLLB_MODELS_DIR, help="Directory path of the Facebook NLLB model")
